...
class Product(PurchaseProduct,SaleProduct):
	...
	def __init__(self,productName,productType):
		...
		for i in range(0,len(header_lists)):
			purchase_sheet.write(0,i,header_lists[i])  #Write the header in purchase_sheet
			sale_sheet.write(0,i,header_lists[i]) #Write the header in sale_sheet
		for i in range(0,len(product_lists)):	
			product_sheet.write(0,i,product_lists[i])
